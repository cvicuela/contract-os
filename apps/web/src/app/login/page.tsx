...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <nav className="flex items-center justify-between px-4 sm:px-6 py-4 max-w-6xl mx-auto">
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          <button
            ...
            className="text-sm border border-white/15 hover:border-white/30 px-4 py-1.5 rounded-lg text-gray-300 hover:text-white transition-all"
          >
            {demoLoading ? t.loading : t.login.tryLiveDemo}
          </button>
          <button
            onClick={() => signIn("google", { callbackUrl: "/" })}
            className="text-sm font-medium bg-white text-gray-950 px-4 py-1.5 rounded-lg hover:bg-gray-100 transition-colors flex items-center gap-2"
          >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-3.5 h-3.5 shrink-0">
      ...
      {menuOpen && (
        <div className="sm:hidden bg-gray-900 border-b border-white/10 px-4 py-4 space-y-3">
          <button
            onClick={() => { setMenuOpen(false); signIn("google", { callbackUrl: "/" }) }}
            className="w-full text-left text-sm text-gray-300 hover:text-white py-2 transition-colors"
          >
            {t.login.signIn}
          </button>
          <button
            onClick={() => { setMenuOpen(false); handleDemo() }}
            disabled={demoLoading}
            className="w-full text-left text-sm text-gray-300 hover:text-white py-2 transition-colors"
          >
            {demoLoading ? t.loading : t.login.tryLiveDemo}
          ...
          <button
            onClick={() => { setMenuOpen(false); signIn("google", { callbackUrl: "/" }) }}
            className="w-full text-sm font-medium bg-white text-gray-950 px-4 py-2.5 rounded-lg hover:bg-gray-100 transition-colors"
          >
            {t.login.startFreeTrial}
          </button>
          <button
            onClick={() => { setMenuOpen(false); setLocale(locale === 'en' ? 'es' : 'en') }}
            className="w-full text-left text-xs text-gray-500 hover:text-gray-300 py-2 transition-colors"
          >
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pt-12 sm:pt-16 pb-16 sm:pb-20 text-center">
        ...
        <div className="mt-16 max-w-4xl mx-auto">
          <div className="relative rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-1 shadow-2xl shadow-indigo-500/10">
            {/* Browser chrome */}
            <div className="flex items-center gap-1.5 px-3 py-2 border-b border-white/10">
              ...
              <div className="flex-1 mx-8">
                <div className="h-4 bg-white/5 rounded-full max-w-xs mx-auto" />
              </div>
            </div>
            {/* Dashboard preview */}
            <div className="p-4 sm:p-6 space-y-4">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 sm:gap-3">
                {[
                  { label: 'Total', value: '24', color: 'text-indigo-400' },
                  { label: 'Active', value: '18', color: 'text-emerald-400' },
                  { label: 'Expiring', value: '3', color: 'text-amber-400' },
                  { label: 'High Risk', value: '2', color: 'text-red-400' },
                  { label: 'Alerts', value: '5', color: 'text-violet-400' },
                  { label: 'Overdue', value: '1', color: 'text-orange-400' },
                ].map((stat) => (
                  <div key={stat.label} className="bg-white/5 rounded-lg p-2 sm:p-3 text-center">
              ...
              <div className="bg-white/5 rounded-lg overflow-hidden">
                <div className="px-3 py-2 border-b border-white/5 flex items-center justify-between">
                  <span className="text-xs text-gray-400 font-medium">Recent Contracts</span>
                  <span className="text-[10px] text-indigo-400">View all &rarr;</span>
                </div>
                {[
                  { name: 'Office Lease Agreement', risk: 3, status: 'Active', statusColor: 'text-emerald-400' },
                  { name: 'SaaS License — Acme Corp', risk: 7, status: 'Expiring', statusColor: 'text-amber-400' },
                  { name: 'NDA — Partner Inc', risk: 2, status: 'Active', statusColor: 'text-emerald-400' },
                ].map((row) => (
                  <div key={row.name} className="flex items-center justify-between px-3 py-2 border-b border-white/5 last:border-0">
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`text-[10px] font-medium ${row.risk <= 3 ? 'text-emerald-400' : row.risk <= 6 ? 'text-amber-400' : 'text-red-400'}`}>
                        {row.risk}/10
                      </span>
                      <span className={`text-[10px] ${row.statusColor}`}>{row.status}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-gray-950 to-transparent rounded-b-xl pointer-events-none" />
          </div>
        </div>
      </section>
      ...
      <section id="pricing" className="max-w-6xl mx-auto px-4 sm:px-6 pb-24">
        ...
        <div className="flex items-center justify-center gap-3 mb-8">
          ...
          >
            <div className={`absolute top-0.5 w-5 h-5 bg-indigo-500 rounded-full transition-transform ${billing === 'annual' ? 'translate-x-6' : 'translate-x-0.5'}`} />
          </button>
          <span className={`text-sm ${billing === 'annual' ? 'text-white' : 'text-gray-500'}`}>
            {t.login.billingAnnual}
          </span>
          {billing === 'annual' && (
              ...
              <div>
                ...
                <div className="flex items-end gap-1 mb-1">
                  {billing === 'annual' && plan.key !== 'trial' && (
                    <span className="text-sm line-through text-gray-500 mr-1">{planT.price}</span>
                  )}
                  <span className={`text-3xl font-bold ${plan.highlight ? "text-white" : "text-white"}`}>
                    {billing === 'annual' ? planT.annualPrice : planT.price}
                  </span>
                  <span className={`text-sm pb-1 ${plan.highlight ? "text-indigo-200" : "text-slate-400"}`}>
                    /{billing === 'annual' ? planT.annualPeriod : planT.period}
                  </span>
                </div>
        ...
        <div className="mt-6 bg-slate-800 border border-slate-700 rounded-2xl px-6 sm:px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <a
            href="mailto:hello@contractos.app"
      ...
      <footer className="border-t border-white/8 px-6 py-8 max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-gray-600">
        ...
        <div className="flex items-center gap-4 text-gray-600">
          <a href="mailto:hello@contractos.app" className="hover:text-gray-400 transition-colors">{t.login.footer.contact}</a>
          <span>·</span>
          <a href="/terms" className="hover:text-gray-400 transition-colors">{t.login.footer.terms}</a>
          <span>·</span>
          <a href="/privacy" className="hover:text-gray-400 transition-colors">{t.login.footer.privacy}</a>
          <span>·</span>
