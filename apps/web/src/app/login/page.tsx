      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pb-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {FEATURE_KEYS.map((key, i) => (
            <div key={key} className="bg-white/5 border border-white/8 rounded-xl p-5 hover:bg-white/8 transition-colors">
              <div className="w-9 h-9 bg-indigo-500/15 rounded-lg flex items-center justify-center text-indigo-400 mb-4">
                {FEATURE_ICONS[i]}
              </div>
              <h3 className="font-semibold text-sm mb-1.5">{t.login.features[key].title}</h3>
